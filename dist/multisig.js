...
export async function deployMultisig(params) {
    const finalParams = {
        ...params,
    ...
    if (finalParams.selfDeploy) {
        ...
        const udcCalls = defaultDeployer.buildDeployerCall({ classHash, salt, constructorCalldata, unique: false }, accountAddress);
...
export async function deployMultisig1_3(params = {}) {
    return deployMultisig({ ...params, threshold: 1, signersLength: 3 });
}
export async function deployMultisig1_1(params = {}) {
    return deployMultisig({ ...params, threshold: 1, signersLength: 1 });
}
const sortedKeyPairs = (length) => sortByGuid(randomStarknetKeyPairs(length));
const keysToSigners = (keys) => keys.map(({ signer }) => signer);
export async function deployLegacyMultisig(classHash, threshold = 1) {
