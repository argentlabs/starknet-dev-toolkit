...
function isLocalDevnetUrl(url) {
    return url.startsWith("http://127.0.0.1:") || url.startsWith("http://localhost:");
}
async function isDevnetAlive(baseUrl) {
...
async function startDockerDesktopIfNeeded() {
    if (canTalkToDocker())
        return true;
...
export async function ensureDevnet(options = {}) {
    const { cwd = process.cwd(), imageTag = "devnet", containerName = "devnet", port = "127.0.0.1:5050:5050", rpcUrlEnv = "RPC_URL", defaultRpcUrl = DEFAULT_RPC_URL, loadDotenv = false, forkRpcUrlEnv, rm = true, dockerBuildExtraArgs = [], dockerRunExtraArgs = [], } = options;
...
//# sourceMappingURL=ensureDevnet.js.map