import type { BigNumberish, CairoCustomEnum, CairoOption, Call, Calldata, Contract, EstimateFeeResponseOverhead, InvokeFunctionResponse, RawCalldata, Uint256 } from "starknet";
...
type PopulateTransactionFor<Methods> = {
    [MethodName in keyof Methods as Methods[MethodName] extends (...args: any[]) => unknown ? MethodName : never]: Methods[MethodName] extends (...args: infer Args) => unknown ? (...args: Args) => Call : never;
};
type EstimateFeeFor<Methods> = {
    [MethodName in keyof Methods as Methods[MethodName] extends (...args: any[]) => unknown ? MethodName : never]: Methods[MethodName] extends (...args: infer Args) => unknown ? (...args: Args) => Promise<EstimateFeeResponseOverhead> : never;
};
...
export type ArgentAccountContract = ContractWithPopulate<ArgentBaseAccountMethods & {
    ...
    trigger_escape_guardian: (payload: BigNumberish | RawCalldata | CairoOption<bigint>) => Promise<InvokeFunctionResponse>;
...
export type ArgentMultisigContract = ContractWithPopulate<ArgentBaseAccountMethods & {
