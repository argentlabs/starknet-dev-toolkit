...
export function WithContracts(Base) {
    return class extends Base {
        ...
        async declareLocalContract(contractName, wait = true, folder = contractsFolder) {
            ...
            if (Object.keys(this.cacheClassHashes).length === 0) {
                ...
                this.cacheClassHashes = readJsonFile(cacheClassHashFilepath);
            }
        ...
        async declareArtifactAccountContract(contractVersion, wait = true) {
        ...
        async declareArtifactMultisigContract(contractVersion, wait = true) {
            const allArtifactsFolders = getSubfolders(artifactsFolder);
        ...
        async loadContract(contractAddress, classHash) {
        ...
        async declareAndDeployContract(contractName, payload, details) {
...
function resolveContractFile(contractName, folder) {
    ...
    if (matches.length === 1)
        return resolve(absoluteDir, matches[0]);
    if (matches.length === 0) {
        throw new Error(`No file matching "*${target}" found in ${absoluteDir}`);
    }
    const exact = matches.find((f) => f === target);
    if (exact)
        return resolve(absoluteDir, exact);
    throw new Error(`Multiple files match "${target}" in ${absoluteDir}: ${matches.join(", ")}. Pass a path that uniquely identifies the contract.`);
}
// This has to be fast. We don't care much about collisions
async function hashFileFast(filePath) {
